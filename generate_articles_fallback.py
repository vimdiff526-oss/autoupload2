"""
Fallback-enabled script to fetch transcripts from YouTube, summarise them and
generate Markdown articles. This version is based on the original
``generate_articles.py`` but modifies the transcript retrieval logic so that it
will work with older versions of ``youtube-transcript-api`` where
``list_transcripts`` is not available. If ``list_transcripts`` exists it will
be used to locate transcripts in preferred languages. Otherwise the code
falls back to ``get_transcript`` which is available in earlier releases.

Usage::

    # Ensure the YOUTUBE_API_KEY environment variable is set
    export YOUTUBE_API_KEY=<your API key>
    python generate_articles_fallback.py

The configuration for the script is stored in ``config.json``.

Note:
    - This script does not perform any authentication against note.com; instead
      it focuses purely on gathering and summarising content.
...
# Default language for Sumy summarisation
LANGUAGE = "english"
...
def resolve_channel_id(youtube, identifier: str) -> str:
    ...
    "@" it is treated as a handle and the corresponding channel ID is looked
    up via the search API. Otherwise it is searched for as a channel name.
    """
...
def fetch_transcript(video_id: str, languages: List[str] = None) -> str:
    ...
    The function first attempts to use ``YouTubeTranscriptApi.list_transcripts`` to
    find transcripts in the preferred languages. If that method is not
    available (e.g. in older versions of the library) or raises an
    ``AttributeError``, it falls back to ``YouTubeTranscriptApi.get_transcript`` which
    returns the transcript directly.
    """
    if languages is None:
        languages = ["ja", "en"]  # try Japanese then English
    # Try to use list_transcripts if available
    try:
        transcript_list = YouTubeTranscriptApi.list_transcripts(video_id)
    except AttributeError:
        # The library does not provide list_transcripts; fallback to get_transcript
        ...
        # If no transcripts found via list_transcripts, try get_transcript as fallback
        for lang in languages:
            try:
                segments = YouTubeTranscriptApi.get_transcript(video_id, languages=[lang])
                return " ".join(segment["text"] for segment in segments)
        ...
        try:
            ...
            return None
    # At this point we have transcript_list
    transcript = None
    for lang in languages:
        try:
